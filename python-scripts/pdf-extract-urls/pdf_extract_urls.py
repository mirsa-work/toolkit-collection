...
exclude_patterns = [
    ...
    re.compile(r"^(?!https?://(www\.)?example\.)")  # Exclude all URLs where the second-level domain is not "example"
]
...
with open('unique_urls.csv', 'w', newline='') as csvfile:
...
with open('file_url_map.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
...
with open('excluded_urls.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
...
with open('unique_excluded_urls.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Unique Excluded URLs"])
    for url in sorted(unique_excluded_urls):
        writer.writerow([url])

print("CSV files 'unique_urls.csv', 'file_url_map.csv', 'excluded_urls.csv', and 'unique_excluded_urls.csv' have been generated.")
if failed_files:
